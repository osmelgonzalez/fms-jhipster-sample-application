    ...
    <div>
      ...
      <div className="table-responsive">
        {checkinList && checkinList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  Player <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {checkinList.map((checkin, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{checkin.timestamp ? <TextFormat type="date" value={checkin.timestamp} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{checkin.player ? <Link to={`/player/${checkin.player.id}`}>{checkin.player.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/checkin/${checkin.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Checkins found</div>
        )}
      </div>
      {totalItems ? (
